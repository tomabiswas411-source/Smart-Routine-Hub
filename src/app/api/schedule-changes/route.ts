...
import type { ScheduleChange, Notice } from "@/lib/firebase-services";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (newRoomId) {
      ...
      newRoomNumber = newRoom?.roomNumber || null;
    }
    ...
    const scheduleChange = await createScheduleChange(removeUndefined(changeData) as Omit<ScheduleChange, "id" | "createdAt">);
    ...
    await createNotice(removeUndefined(noticeData) as Omit<Notice, "id" | "createdAt" | "updatedAt">);
